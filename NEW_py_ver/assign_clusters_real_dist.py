...
# --- Parameters ---
depot_ids = ['2501', '25P2', '2522', '25P1']  # USE NG
colors = {'2501': 'red', '25P2': 'blue', '2522': 'purple', '25P1': 'green'}
#depot_ids = ['2501', '2513', '2522', '2526']  # USE Taguig 3
...
customer_files = glob.glob('./clean_data/merged_transport_orders_unique.csv')

def assign_depot(lat, lon, depot_centers, depot_ids, cluster_sizes, max_customers_per_depot):
...
for file in customer_files:
    ...
    df['AssignedDepotID'] = df.apply(
        lambda row: assign_depot(row['Latitude'], row['Longitude'], depot_centers, depot_ids, cluster_sizes, max_customers_per_depot),
        axis=1
    )
...
map_filename = f'map_{dt_str}_{max_customers_per_depot}.html'
m.save(map_filename)
